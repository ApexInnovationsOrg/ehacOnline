...
function ConvertCMITimeSpanToMS(strTime){
	...
	WriteToDebug("In ConvertSCORMTimeToMS, strTime=" + strTime);
	...
	for (var i=1; i < strIso8601Time.length; i++){
		...
		if (IsIso8601SectionDelimiter(strCurrentCharacter)){
			...
			switch (strCurrentCharacter.toUpperCase()){
				...
				case "Y":
					Years = parseInt(strNumberBuilder);
				break;
				
				case "M":
					if (blnInTimeSection){
						Minutes = parseInt(strNumberBuilder);
					}
					else{
						Months = parseInt(strNumberBuilder);
					}
				break;
				
				case "D":
					Days = parseInt(strNumberBuilder);
				break;
				
				case "H":
					Hours = parseInt(strNumberBuilder);
				break;
...
function ConvertMilliSecondsToSCORMTime(intTotalMilliseconds){
	...
	WriteToDebug("In ConvertMilliSecondsIntoSCORMTime, intTotalMilliseconds=" + intTotalMilliseconds);
